...
import {
  DEMO_CAMPUS_JOB_CONVERSATION_ID,
  openDemoCampusJobConversation,
} from "@/lib/services/messaging-service";
import { getOrCreateCampusJobConversation } from "@/lib/services/supabase-messaging-service";
...
interface MessageJobPosterButtonProps
...
export function MessageJobPosterButton({
  jobId,
  posterId,
  jobTitle,
  redirectPath,
  ...
  const returnPath = redirectPath ?? `/jobs/${jobId}`;
  ...
  const handleClick = async (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (demoMessaging) {
      const demo = await openDemoCampusJobConversation(jobId);
      setLoading(false);
    ...
    const result = await getOrCreateCampusJobConversation(jobId, user.id);
    setLoading(false);
  ...
  return (
    <div className={cn("space-y-2", className?.includes("w-full") && "w-full")}>
      <Button
        ...
        data-testid={`message-job-poster-${jobId}`}
        {...props}
      >
...
export { DEMO_CAMPUS_JOB_CONVERSATION_ID };
