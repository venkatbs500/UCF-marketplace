...
import {
  ...
  MARKETPLACE_BROWSE_SORT_OPTIONS,
...
export function MarketplaceSearchControls({
  ...
  const sortLabel =
    MARKETPLACE_BROWSE_SORT_OPTIONS.find((o) => o.id === state.sort)?.label ?? "Newest";
  ...
  const filtersActive = isMarketplaceFilterActive(state);

  return (
    <div className="space-y-4" data-testid="marketplace-search-controls">
      <SearchBar
          ...
          <div className="relative">
            ...
            {sortOpen && (
              <div className="absolute right-0 top-full z-20 mt-2 w-48 rounded-2xl glass-card border border-white/10 p-2 shadow-xl">
                {MARKETPLACE_BROWSE_SORT_OPTIONS.map((opt) => (
                  <button
                    key={opt.id}
                    type="button"
                    onClick={() => {
