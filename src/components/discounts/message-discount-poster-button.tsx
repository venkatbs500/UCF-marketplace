...
import {
  DEMO_STUDENT_DISCOUNT_CONVERSATION_ID,
  openDemoStudentDiscountConversation,
} from "@/lib/services/messaging-service";
import { getOrCreateStudentDiscountConversation } from "@/lib/services/supabase-messaging-service";
...
interface MessageDiscountPosterButtonProps
...
export function MessageDiscountPosterButton({
  discountId,
  posterId,
  discountTitle,
  redirectPath,
  ...
  size = "default",
  ...props
}: MessageDiscountPosterButtonProps) {
  ...
  const returnPath = redirectPath ?? `/discounts/${discountId}`;
  ...
  const handleClick = async (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (demoMessaging) {
      const demo = await openDemoStudentDiscountConversation(discountId);
      setLoading(false);
      const params = new URLSearchParams({ conversation: demo.conversationId });
      if (discountTitle) {
        params.set("discount", discountTitle);
      }
    ...
    const result = await getOrCreateStudentDiscountConversation(discountId, user.id);
    setLoading(false);
  ...
  return (
    <div className={cn("space-y-2", className?.includes("w-full") && "w-full")}>
      <Button
        ...
        data-testid={`message-discount-poster-${discountId}`}
        {...props}
      >
...
export { DEMO_STUDENT_DISCOUNT_CONVERSATION_ID };
